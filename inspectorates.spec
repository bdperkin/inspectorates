...
for i in $(grep '^=head1 ' %{name} | awk '{print $2}'); do echo -n "$i => "; j=$(echo $i | sed -e 's/B<//g' | sed -e 's/>//g' | tr [:lower:] [:upper:]); echo $j; sed -i -e "s/$i/$j/g" %{name}; k=$(echo $i | sed -e 's/>//g' | tr [:lower:] [:upper:]); echo $k; sed -i -e "s/$i/$k/g" %{name}; done
